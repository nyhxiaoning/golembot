...
try {
  ...
  const spDir = await mkdtemp(join(tmpdir(), 'golem-cdx-sp-'));
  ...
  try {
    ...
    await spBot.init({ engine: 'codex', name: 'sp-test-bot' });
  ...
  console.log(`    File I/O + Skill (AGENTS.md) + memory + IM Bot + CI/CD + HTTP service + systemPrompt${RESET}\n`);
